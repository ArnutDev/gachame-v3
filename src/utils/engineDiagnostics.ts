 ...
 */
export function runEngineUnitTests(
  banner: Banner,
  pool: (Ranger | Gear)[]
): TestCaseResult[] {
  ...
  assertTest(
    ...
    () => {
      ...
      const outcome2 = rollGacha(mockBanner, pool, 0, () => 0.8);
      ...
      const outcome4 = rollGacha(mockBanner, pool, 0, () =>
        isGear ? 0.98 : 0.99
      );
  ...
  assertTest(
    ...
    () => {
      ...
      return (
        outcome1.item.id === 'single_item' && outcome2.item.id === 'single_item'
      );
    },
  ...
  assertTest(
    ...
    () => {
      ...
      return (
        !validation.isValid &&
        validation.errors.some((e) => e.includes('must sum to 100%'))
      );
    },
    ...
    () => {
      ...
      return (
        ...
        validation.errors.some((e) =>
          e.includes('exceeds the total rarity rate')
        )
      );
    },
  ...
  assertTest(
    ...
    () => {
      ...
      const matches = rolls1.every(
        (o, idx) => o.item.id === rolls2[idx].item.id
      );
      return matches;
    },
...
export function runEngineBenchmark(
  ...
  const pullsPerSecond =
    timeMs > 0 ? (pullCount / timeMs) * 1000 : pullCount * 1000;
...
export function generateEngineStatisticalReport(
  ...
  const rarityStats = relevantRarities.map((r) => {
    ...
    const actual =
      pullCount > 0 ? ((rarityCounts[r] || 0) / pullCount) * 100 : 0;
    return {
      rarity: r,
      expected,
      actual,
  ...
  const itemStats = pool.map((item) => {
    ...
    const actual =
      pullCount > 0 ? ((itemCounts[item.id] || 0) / pullCount) * 100 : 0;
